...
export function PortfolioContent() {
  ...
  const [selectedProject, setSelectedProject] = useState<GhostPost | null>(null)
      ...
      <div style={wrapperStyle}>
        ...
        {detailStatus === 'loading' && (
          <div style={detailLoadingStyle}>
            <span className="articles-spinner" role="status" aria-label={PORTFOLIO_CONTENT.detailLoadingMessage} />
          </div>
        )}
        ...
        {detailStatus === 'error' && <p style={errorStyle}>{PORTFOLIO_CONTENT.detailErrorMessage}</p>}
      </div>
    )
  }
    ...
    <div>
      ...
      <div style={wrapperStyle}>
        <table style={tableStyle}>
          <thead>
            <tr>
              <th style={{ ...headerCellStyle, ...headerCellDividerStyle }}>{PORTFOLIO_CONTENT.columnHash}</th>
              <th style={{ ...headerCellStyle, ...headerCellDividerStyle }}>{PORTFOLIO_CONTENT.columnProject}</th>
              <th style={{ ...headerCellStyle, ...headerCellDividerStyle }}>{PORTFOLIO_CONTENT.columnYear}</th>
          ...
          {status === 'loading' && (
            <tbody>
              {Array.from({ length: PORTFOLIO_SKELETON_ROW_COUNT }).map((_, index) => (
                <tr key={index}>
                  ...
                  <td style={{ ...cellStyle, ...cellDividerStyle }}>
                    <span className="articles-skeleton-bar" style={skeletonProjectBarStyle} />
                  </td>
                  <td style={{ ...cellStyle, ...cellDividerStyle }}>
                    <span className="articles-skeleton-bar" style={skeletonYearBarStyle} />
                  </td>
                  <td style={cellStyle}>
                    <span className="articles-skeleton-bar" style={skeletonCompanyBarStyle} />
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          {status === 'success' && posts.length > 0 && (
            <tbody>
              {posts.map((post, index) => (
                <tr key={post.id} className="articles-row" style={rowStyle} onClick={() => openProject(post)}>
                  <td style={{ ...cellStyle, ...cellDividerStyle, ...hashCellStyle }}>{index + 1}</td>
                  <td style={{ ...cellStyle, ...projectCellStyle, ...cellDividerStyle }}>
                    <button type="button" style={projectButtonStyle} onClick={() => openProject(post)}>
                      {post.title}
                    </button>
                  </td>
                  <td style={{ ...cellStyle, ...cellDividerStyle }}>{getProjectYear(post)}</td>
                  <td style={cellStyle}>{getProjectCompany(post)}</td>
                </tr>
              ))}
            </tbody>
          )}
        ...
        {status === 'error' && <p style={errorStyle}>{PORTFOLIO_CONTENT.errorMessage}</p>}
      </div>
    </div>
  )
}
